...
def Analysis_free():
    ...
    if 'selected_date' not in st.session_state:
        ...
        st.session_state['selected_date'] = pd.Timestamp.now(tz=pytz.timezone('US/Eastern')).date()
    else:
    ...
    if table_timestamps is not None:
        ...
        if 'ticker_selected' not in st.session_state or st.session_state['ticker_selected'] not in ticker_options:
            ticker_selected = st.sidebar.selectbox('Ticker', options=ticker_options, key='ticker_init', on_change=ticker_init,disabled=True)
            
        elif 'ticker_selected' in st.session_state and st.session_state['ticker_selected'] in ticker_options:
            ticker_selected = st.sidebar.selectbox('Ticker', options=ticker_options, key='ticker_sel', on_change = ticker_select, index=ticker_options.index(st.session_state['ticker_selected']),disabled=True)
